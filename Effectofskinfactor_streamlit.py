...
rw = st.sidebar.number_input('Wellbore Radius (ft)', min_value=0.1, value=3.0)
...
def calculate_pressure_profile(skin):
    ...
    P = []
    for i in rskin:
        a = pe - (141.2 * q * mu * Bo * (np.log(re / i) + float(skin)) / k / h)
        P.append(a)
    A = np.array(P)
...
skin = st.slider('Formation Damage (Skin)', min_value= -5.0, max_value=5.0, value=1.0)
